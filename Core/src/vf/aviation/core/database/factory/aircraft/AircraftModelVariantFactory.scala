...
object AircraftModelVariantFactory extends FromValidatedRowModelFactory[AircraftModelVariant]
{
	...
	override protected def fromValidatedModel(model: Model[Constant]) = AircraftModelVariant(model("id"),
		AircraftModelVariantData(model("modelId"), model("manufacturerId"), model("name"), model("manufacturerCode"),
			model("modelCode"), model("seriesCode"), model("designGroupId"),
			model("wingSpanFeet").double.map(Distance.ofFeet), model("lengthFeet").double.map(Distance.ofFeet),
